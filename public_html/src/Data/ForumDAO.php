...
class ForumDAO extends DBConfig
{
    ...
    public function getTopicData()
    {
        ...
        $statement = $this->dbh->prepare("
            SELECT
                t.`id`,t.`title`, t.`content`,u.`userStyle`, u.`username` AS `starter`, u.`forumPosts`, s.`id` AS `statusID`, d.`id` AS `donatorID`, t.`starterUID`, s.`status_".$this->lang."` AS `status`,
            ...
            )
            {
                if(($row['familyForum'] == 1 && is_object($userData) && $userData->getFamilyID() != 0) || $row['familyForum'] == 0)
                {
                    ...
                    )
                    {
                        ...
                        $forum->setUserStyle($row['userStyle']);
    ...
    public function getTopicDataByTopicId($id)
    {
        ...
        $statement = $this->dbh->prepare("
            SELECT
                t.`id`,t.`title`, t.`content`,u.`userStyle`, u.username AS `starter`, u.`forumPosts`, s.`id` AS `statusID`, d.`id` AS `donatorID`, s.`status_".$this->lang."` AS `status`,
    ...
    public function getTopicReactions($from, $to)
    {
        ...
        {
            $statement = $this->dbh->prepare("
                SELECT r.`id`,r.`content`, r.`userID`, DATE_FORMAT( r.`date`, '".$this->dateFormat."' ) AS `date`, r.`lastEditTime`, u.`username` AS `reactor`,u.`userStyle`, u.`avatar`, u.`forumPosts`,
            ...
            foreach($statement AS $r)
            {
                ...
                $reaction->setUserStyle($r['userStyle']);
    ...
    public function getTopicReactionsByTopicId($topicID, $from, $to)
    {
        ...
        {
            $statement = $this->dbh->prepare("
                SELECT r.`id`,r.`content`, r.`userID`, DATE_FORMAT( r.`date`, '".$this->dateFormat."' ) AS `date`, r.`lastEditTime`,u.`userStyle`, u.`username` AS `reactor`, u.`avatar`, u.`forumPosts`,
                s.`id` AS `statusID`,d.`id` AS `donatorID`, s.`status_".$this->lang."` AS `status`, d.`donator_".$this->lang."` AS `donator`, r.`lastEditTime`, c.`viewStatusID`
                FROM `forum_reaction` AS r
                LEFT JOIN `forum_topic` AS t
                ON (r.`topicID`=t.`id`)
