...
class AdminDAO extends DBConfig
{
    ...
    public function getTableRows($from = 0, $to = 15, $onlyFields = FALSE)
    {
        if(isset($_SESSION['cp-logon']) && in_array($this->table,$this->validTables))
        {
            ...
            //Only show rows (messages) that have been reported.
            if(in_array($this->table, ["shoutbox_nl", "shoutbox_en"])){
                $fieldWhere = " AND `reports` > 0 ";
            }else
            $fieldWhere = " ";


            $statement = $this->dbh->prepare("SELECT $fields FROM `$this->table` WHERE `deleted` = '0' $fieldWhere ORDER BY `position` ASC, `id` ASC LIMIT $from,$to");
            $statement->execute();
            $list = array();
            $list = $this->checkFieldsComments($list, $statement);
            return $list;
