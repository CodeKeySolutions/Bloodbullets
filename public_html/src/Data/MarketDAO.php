...
class MarketDAO extends DBConfig
{
    ...
    public function getMarketOffersByType($typeID)
    {
        if(isset($_SESSION['UID']))
        {
            ...
            $statement = $this->dbh->prepare("
                ...
                LEFT JOIN `user` AS u
                ON (m.userID = u.id)
                WHERE `type`= :typeID AND `requested`='0' AND m.`deleted` = 0 AND m.`active` = 1
            ...
            $statement = $this->dbh->prepare("SELECT `id` FROM `market` WHERE `active` = 1 AND  `deleted` = 0 ORDER BY `id` DESC LIMIT 1");
            $statement->execute();
            $row = $statement->fetch();
            $lastMarketId = $row['id'] ?? 0;

            $statement = $this->dbh->prepare("UPDATE `user` SET `lastMarketID`= :lastMarketId WHERE `id`= :uid");
            $statement->execute(array(':lastMarketId' => $lastMarketId, ':uid' => $_SESSION['UID']));
    ...
    public function getMarketRequestsByType($typeID)
    {
        if(isset($_SESSION['UID']))
        {
            $statement = $this->dbh->prepare("
                ...
                LEFT JOIN `user` AS u
                ON (m.userID = u.id)
                WHERE `type`= :typeID AND `requested`='1' AND m.`deleted` = 0 AND m.`active` = 1
                ORDER BY m.`date` DESC
            ");
            $statement->execute(array(':typeID' => $typeID));
    ...
    public function lastMarketItem() : int
    {
        if(isset($_SESSION['UID']))
        {
            $statement = $this->dbh->prepare("SELECT `id` FROM `market` WHERE `active` = 1 AND  `deleted` = 0  ORDER BY `id` DESC LIMIT 1");
