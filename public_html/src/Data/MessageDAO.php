...
class MessageDAO extends DBConfig
{
    ...
    public function muteSender($userId)
    {
        if(!isset($_SESSION['UID'])) return;
        $stmt = $this->dbh->prepare("UPDATE `message` SET `mute` =DATE_ADD(NOW(), INTERVAL 1 HOUR) WHERE `senderID` = :userID ");
        $stmt->execute(array(":userID" => $userId));
    }
    public function senderIsMuted()
    {
        if(!isset($_SESSION['UID'])) return FALSE;

        $stmt = $this->dbh->prepare("SELECT `mute` FROM `message` WHERE `senderID` = :senderID AND `active` = 1 AND `deleted` = 0 AND `mute` > NOW() ");
        $stmt->execute(array(':senderID'=> $_SESSION['UID']));
        $result = $stmt->fetch();
