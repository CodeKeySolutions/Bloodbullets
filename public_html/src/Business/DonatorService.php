...
class DonatorService extends DonatorStatics
{
    ...
    public function interactDonationShop($post)
    {
        ...
        $userStyle1 = isset($post['username-style']) ? true : null;
        $userStyle = isset($post['style']) ? $_POST['style'] : 0;
        ...
        else
        {
            ...
            elseif($userStyle1 )
            {
                if($userData->getUserStyle() == $userStyle)
                {
                    $replacedMessage = $l['BOUGHT_STYLE_ALREADY_OWNED'];
                }else {
                $this->data->buyuserStyle($creditsNeeded, $userStyle);
                $replacedMessage = $l['BOUGHT_NICKNAME_SUCCESS'];
                }
            }
    ...
    { // Ignore security-token on purpose to avoid possible failed donations for an expired token.
        ...
        global $langs;
        $l = $language->donationShopLangs();
        $startEvent = "2025-03-21 00:00:00";
        $endEvent = "2025-03-24 00:00:00";
        ...
        else
        {
            ...
            if(is_object($json) && isset($json->status) && is_string($json->status) && strtoupper($json->status) === "COMPLETED")
            {
                ...
                {
                    if(strtotime(HAPPY_HOUR_START) < strtotime('now') && strtotime(HAPPY_HOUR_END) > strtotime('now'))
                        $cr *= 2;
