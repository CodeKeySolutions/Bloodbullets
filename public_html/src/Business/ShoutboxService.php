...
class ShoutboxService
{
    ...
    public function postMessage($post)
    {
        ...
        global $security;
        $famID           = (int)$_POST['famID'] ?? 0;
        $message = $security->xssEscape($post['message']);
        $messageType = isset($_POST['type']) ? (int)$_POST['type'] : 0;
        ...
        if($this->isUserMuted($_SESSION['UID'])){
            $error = $l['IS_MUTED'];
        }
        ...
        {
            $this->data->postMessage($famID, $message, $messageType);
    ...
    public function reportMessage($reportID){
        global $route;
        $this->data->reportMessage($reportID);
        $route->headTo("shoutbox");
    }

    public function muteUserById($userId)
    {
        global $route;
        $this->data->muteUserById($userId);
        $route->headTo("shoutbox");
    }
    ...
    {
        global $route;
        $this->data->deleteMessageById($messageId);
        $route->headTo("shoutbox");
    }

    public function isUserMuted($userId)
    {
        return $this->data->isUserMuted($userId);
    }
}
